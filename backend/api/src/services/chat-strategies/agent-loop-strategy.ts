...
import { TOOL_RESULT_COMPACTION, LOOP_DETECTION, PRE_COMPLETION_CHECKLIST, TRACE_ANALYZER, EVAL_PIPELINE, CONTEXT_GC, INFORMED_FALLBACK } from '../../config/runtime-limits';
        ...
        while (currentTurn < context.maxTurns) {
            ...
            if (directResult.metrics) {
                ...
                for (const key of ADDITIVE) {
                    ...
                    const cur = typeof m[key] === 'number' ? m[key] as number : 0;
                    if (prev || cur) merged[key] = prev + cur;
                }
                metrics = merged;
            }
            ...
            if (directResult.toolCalls.length > 0) {
                ...
                for (const toolCall of directResult.toolCalls) {
                    ...
                    const toolResult = await executeToolCall(context, toolCall);
