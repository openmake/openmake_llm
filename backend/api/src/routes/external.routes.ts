 ...
 * 특정 서비스 연결 조회
 */
router.get('/:serviceType', requireAuth, asyncHandler(async (req: Request, res: Response) => {
 ...
 * 서비스 연결 해제
 */
router.delete('/:serviceType', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    const { serviceType } = req.params;
 ...
 * 캐시된 외부 파일 목록 조회
 */
router.get('/:connectionId/files', requireAuth, asyncHandler(async (req: Request, res: Response) => {
    const { connectionId } = req.params;
    const db = getUnifiedDatabase();
