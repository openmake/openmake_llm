...
router.post('/', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        logger.error('캔버스 문서 생성 오류:', error);
...
router.get('/', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        logger.error('캔버스 문서 목록 조회 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
...
router.get('/shared/:shareToken', async (req: Request, res: Response) => {
    try {
        ...
        logger.error('공유 문서 조회 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
...
router.get('/:documentId', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        logger.error('캔버스 문서 조회 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
 ...
 * 캔버스 문서 수정
 */
router.put('/:documentId', requireAuth, async (req: Request, res: Response) => {
    try {
        const { documentId } = req.params;
        ...
        logger.error('캔버스 문서 수정 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
 ...
 * 캔버스 문서 공유 (공유 토큰 생성)
 */
router.post('/:documentId/share', requireAuth, async (req: Request, res: Response) => {
    try {
        const { documentId } = req.params;
        const db = getUnifiedDatabase();
        ...
        logger.error('캔버스 문서 공유 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
 ...
 * 캔버스 문서 공유 해제
 */
router.delete('/:documentId/share', requireAuth, async (req: Request, res: Response) => {
    try {
        const { documentId } = req.params;
        const db = getUnifiedDatabase();
        ...
        logger.error('캔버스 문서 공유 해제 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
 ...
 * 캔버스 문서 버전 히스토리 조회
 */
router.get('/:documentId/versions', requireAuth, async (req: Request, res: Response) => {
    try {
        const { documentId } = req.params;
        const db = getUnifiedDatabase();
        ...
        logger.error('캔버스 버전 조회 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
...
router.delete('/:documentId', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        logger.error('캔버스 문서 삭제 오류:', error);
        res.status(500).json(internalError(String(error)));
    }
});
