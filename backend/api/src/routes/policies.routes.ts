 ...
 * - 파일 위치: `frontend/web/public/policies/{type}.{locale}.md`
...
/**
 * 정책 파일 디렉토리 — backend/api/dist 또는 src 에서 frontend/web/public/policies 까지 상대 경로.
 * dist 빌드 시 sync-frontend 가 frontend/web/public 을 backend/api/dist/public 으로 복사.
 * 따라서 production 에서는 dist/public/policies, dev 에서는 frontend/web/public/policies.
 */
function getPoliciesDir(): string {
    // 1순위: dist/public/policies (production)
    const distPath = path.join(__dirname, '..', 'public', 'policies');
    // 2순위: frontend/web/public/policies (dev)
