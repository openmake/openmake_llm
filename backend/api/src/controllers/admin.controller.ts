...
export class AdminController {
    ...
    private setupRoutes(): void {
        ...
        this.router.get('/alerts/history', listAlertHistory);
        // alert_history CSV export (운영자 reporting/compliance)
        this.router.get('/alerts/export', exportAlertHistoryCsv);
        // alert_history 통계 (대시보드 요약 카드용)
        this.router.get('/alerts/stats', getAlertStats);
        // LLM Model Pool routing 통계 (PR #98 follow-up)
        this.router.get('/llm-pool/stats', getLlmPoolStats);
        // alert_history acknowledge (확인 처리) — 운영자 ID/시간 기록
        this.router.post('/alerts/:id/acknowledge', acknowledgeAlert);
