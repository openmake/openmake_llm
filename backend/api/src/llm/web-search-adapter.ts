 ...
 * ============================================================
 *
 * Ollama Cloud /api/web_search, /api/web_fetch 는 vLLM 에 존재하지 않으므로
 ...
 *
 * 응답 형식은 기존 OllamaClient.webSearch()/webFetch() 시그니처 호환 유지.
 *
...
export async function webSearch(query: string, maxResults = 5): Promise<WebSearchResponse> {
    try {
        ...
        logger.warn(`webSearch 위임 실패 (Google CSE/Ollama 미설정?): ${e}`);
