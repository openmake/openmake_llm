...
export class RequestQueue<T = any> extends EventEmitter {
    ...
    private async processQueue(): Promise<void> {
        ...
        while (this.size > 0 || this.processing.size > 0) {
            // 동시성 제한 확인
...
// 싱글톤 인스턴스
let chatQueue: RequestQueue | null = null;

export function getChatRequestQueue(processor?: (request: any) => Promise<any>): RequestQueue | null {
    if (!chatQueue && processor) {
        chatQueue = createChatRequestQueue(processor);
    }
    return chatQueue;
}
