...
export async function routeToAgent(message: string, useLLM: boolean = true): Promise<AgentSelection> {
    ...
    // 🆕 LLM 기반 라우팅 시도 (우선순위 1)
    if (useLLM) {
        try {
            const llmResult = await routeWithLLM(message, 5000);
            if (llmResult && llmResult.confidence > 0.5 && isValidAgentId(llmResult.agentId)) {
                const agent = getAgentById(llmResult.agentId);
                if (agent) {
                    console.log(`[Agent Router] ✅ LLM 라우팅 성공: ${agent.name}`);
                    return {
...
/**
 * 🆕 토론용 관련 에이전트 추천 (제한 완화: 더 많은 에이전트 반환)
 */
export async function getRelatedAgentsForDiscussion(message: string, count: number = 10): Promise<Agent[]> {
    const topicAnalysis = analyzeTopicIntent(message);
    const selection = await routeToAgent(message, false); // LLM 미사용 (속도 위해)
    const result: Agent[] = [];
    ...
    // 2. 의도 분석 기반 에이전트 추가 (🆕 제한 없이 모두 추가)
    ...
    // 4. 보완적 관점을 제공할 에이전트 추가 (다른 카테고리에서)
    const complementaryAgents = [
        'business-strategist',  // 비즈니스 관점
        'data-analyst',         // 데이터 관점
        'project-manager',      // 관리 관점
        'financial-analyst',    // 재무 관점
        'risk-manager'          // 리스크 관점
    ];

    for (const agentId of complementaryAgents) {
        if (usedIds.has(agentId)) continue;
        const agent = getAgentById(agentId);
        if (agent) {
            result.push(agent);
