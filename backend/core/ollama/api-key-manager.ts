 ...
 * 🆕 4개 API 키 자동 순환 로직 (backend/api와 동일)
 */
...
export class ApiKeyManager {
    ...
    constructor(config?: Partial<ApiKeyConfig>) {
        ...
        const key3 = process.env.OLLAMA_API_KEY_3 || '';
        const key4 = process.env.OLLAMA_API_KEY_4 || '';

        // config.keys가 있으면 사용, 없으면 환경변수에서 로드
        if (config?.keys && config.keys.length > 0) {
            this.keys = config.keys.filter(k => k && k.trim() !== '');
        } else {
            this.keys = [key1, key2, key3, key4].filter(k => k && k.trim() !== '');
        }
