...
class ApiUsageTracker {
    ...
    private getKeysQuotaStatus(): { primary: KeyQuotaStatus; secondary: KeyQuotaStatus } {
        ...
        try {
            ...
            activeKeyIndex = status.activeKeyIndex || 0;
        } catch (e) {
            // ignore
        }

        return {
            primary: this.getKeyQuotaStatus(getKeyId(key1), activeKeyIndex === 0),
            secondary: this.getKeyQuotaStatus(getKeyId(key2), activeKeyIndex === 1)
        };
    }
