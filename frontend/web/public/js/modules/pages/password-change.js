        ...
        init: function () {
            try {
                ...
                function toggleVis(id, btn) {
                    ...
                    if (inp.type === 'password') { inp.type = 'text'; btn.innerHTML = '<iconify-icon icon=lucide:eye-off></iconify-icon>'; }
                    else { inp.type = 'password'; btn.innerHTML = '<iconify-icon icon=lucide:eye></iconify-icon>'; }
                }
                ...
                // role 은 SafeStorage user 에서 직접 판별 (app state 채움 타이밍 비의존)
                var _acctUser = null;
                try { _acctUser = JSON.parse(SS.getItem(SK.USER || 'user') || 'null'); } catch (e) { }
                if (_acctUser && (_acctUser.role === 'admin' || _acctUser.role === 'administrator')) {
                ...
                async function loadConsents() {
                    ...
                    try {
                        ...
                        if (!res.ok || !data.success) {
                            listEl.innerHTML = '<p style="color: var(--danger); font-size: var(--font-size-sm);">동의 상태 조회 실패</p>';
                            return;
                        }
                        ...
                        listEl.innerHTML = consents.map(function (c) {
                            ...
                            var withdrawBtn = c.granted
                                ? '<button class="btn-submit" style="margin:0;width:auto;font-size:var(--font-size-sm);padding:4px 10px;" data-consent-withdraw="' + esc(c.type) + '">철회</button>'
                                : '';
                            return '<div style="display: flex; align-items: center; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border-light);">'
                                + '<div><strong>' + typeLabel + '</strong>' + versionInfo + ' &nbsp;' + statusBadge + '<div style="font-size: var(--font-size-sm); color: var(--text-muted);">' + dateInfo + '</div></div>'
                                + '<div>' + withdrawBtn + '</div>'
                                + '</div>';
                        }).join('');
                        ...
                        listEl.innerHTML = '<p style="color: var(--danger); font-size: var(--font-size-sm);">동의 상태 조회 실패: ' + esc(String(e.message || e)) + '</p>';
                    }
                }
