...
function init() {
        ...
        // A/B 테스트 시작 폼 렌더링
        function renderABTestForm(agentList) {
            const formEl = document.getElementById('abtestForm');
            if (!formEl) return;
            const options = agentList.map(a => `<option value="${esc(a.id || a.agentId)}">${esc(a.name || a.id)}</option>`).join('');
            formEl.innerHTML = `
                <h3 style="margin:0 0 var(--space-3);color:var(--text-primary);font-size:var(--font-size-base)">새 A/B 테스트 시작</h3>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-3);margin-bottom:var(--space-3)">
                    <div>
                        ...
                        <select id="abtestAgentA" style="width:100%;padding:var(--space-2) var(--space-3);background:var(--bg-secondary);border:1px solid var(--border-light);border-radius:var(--radius-md);color:var(--text-primary);font-size:var(--font-size-sm)">
                    ...
                    <div>
                        ...
                        <select id="abtestAgentB" style="width:100%;padding:var(--space-2) var(--space-3);background:var(--bg-secondary);border:1px solid var(--border-light);border-radius:var(--radius-md);color:var(--text-primary);font-size:var(--font-size-sm)">
                            <option value="">에이전트 선택</option>${options}
                        </select>
                    </div>
                </div>
                <button id="abtestStartBtn" style="padding:var(--space-2) var(--space-5);background:var(--accent-primary);color:#fff;border:none;border-radius:var(--radius-md);cursor:pointer;font-size:var(--font-size-sm);font-weight:600">테스트 시작</button>
            `;
            document.getElementById('abtestStartBtn').addEventListener('click', async () => {
                const agentA = document.getElementById('abtestAgentA').value;
                const agentB = document.getElementById('abtestAgentB').value;
                ...
                try {
                    ...
                    if (data.success) {
                        showToast('A/B 테스트가 시작되었습니다', 'success');
                        loadData();
                    } else {
                        showToast(data.error?.message || '시작 실패', 'error');
                    }
                } catch(err) { showToast('A/B 테스트 시작 실패', 'error'); }
            });
        }
