        ...
        function renderServices() {
            document.getElementById('serviceGrid').innerHTML = SERVICES.map(s => {
                ...
                <div class="service-card">
                    <span class="status-badge ${isConnected ? 'status-on' : 'status-off'}">${isConnected ? '연결됨' : '연결 안됨'}</span>
                    <div class="service-icon">${s.icon}</div>
                    <h3>${s.label}</h3>
                    <div class="account">${isConnected ? esc(conn.accountEmail || conn.account_email || conn.accountName || conn.account_name || '연결됨') : '연결되지 않음'}</div>
                    <div class="service-actions">
                        ${isConnected
                            ? `<button class="btn-files" onclick="viewFiles('${conn.id || conn.connectionId}','${s.label}')">파일 보기</button>
                               <button class="btn-disconnect" onclick="disconnect('${s.type}')">연결 해제</button>`
                            : `<button class="btn-connect" onclick="openConnect('${s.type}','${s.label}')">연결하기</button>`
                        }
                    </div>
                </div>`;
            }).join('');
