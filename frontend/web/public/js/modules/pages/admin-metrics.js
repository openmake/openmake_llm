    ...
    window.PageModules['admin-metrics'] = {
        ...
        init: function () {
            try {
                        ...
                        hourlyChart = new Chart(ctx, {
                            ...
                            data: {
                                ...
                                datasets: [{
                                    ...
                                    backgroundColor: cssVar('--viz-1', '#ff6a3d') + 'AA',
                                    borderColor: cssVar('--viz-1', '#ff6a3d'),
                                    borderWidth: 1,
                                    borderRadius: 4
                                }]
                            },
                                ...
                                datasets: [
                                    {
                                        ...
                                        data: data.datasets.requests,
                                        borderColor: cssVar('--viz-1', '#ff6a3d'),
                                        backgroundColor: cssVar('--viz-1', '#ff6a3d') + '2E',
                                        fill: true,
                                        tension: 0.4
                                    },
                                    {
                                        ...
                                        borderColor: cssVar('--danger', '#e5544e'),
                                        backgroundColor: cssVar('--danger', '#e5544e') + '2E',
                                        fill: true,
                                        tension: 0.4
                                    }
                                ]
                ...
                // CSS 토큰 읽기 헬퍼 — Chart.js 는 var() 직접 미지원
                function cssVar(name, fallback) {
                    var v = getComputedStyle(document.documentElement).getPropertyValue(name).trim();
                    return v || fallback;
                }

                // 차트 옵션 공통 (Graphite & Ember 토큰 기반)
                function getChartOptions() {
                    var tickC = cssVar('--text-muted', '#857c6d');
                    var gridC = cssVar('--border-light', 'rgba(245,238,225,0.08)');
                    return {
                        responsive: true,
