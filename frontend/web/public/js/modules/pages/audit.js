    ...
    window.PageModules['audit'] = {
        getHTML: function () {
            return '<div class="page-audit">' +
                ...
                "/* Audit — Graphite & Ember II, Archetype A */\n        .filter-tabs { display:inline-flex; background:var(--bg-secondary); border:1px solid var(--border-light); border-radius:var(--radius-full,999px); padding:3px; gap:0; margin-bottom:var(--space-4); }\n        .filter-tab { display:inline-flex; align-items:center; gap:6px; font-size:12px; font-weight:var(--font-weight-medium); color:var(--text-muted); background:transparent; border:none; padding:6px 14px; border-radius:var(--radius-full,999px); cursor:pointer; }\n        .filter-tab.active { background:var(--bg-card); color:var(--text-primary); box-shadow:0 1px 3px rgba(0,0,0,.15); }\n        .filter-bar { display:flex; gap:var(--space-3); align-items:flex-end; flex-wrap:wrap; margin-bottom:var(--space-5); background:var(--bg-card); border:1px solid var(--border-light); border-radius:var(--r-lg,var(--radius-lg)); padding:var(--space-4); }\n        .filter-bar .fg { display:flex; flex-direction:column; gap:var(--space-1); }\n        .filter-bar label { font-family:var(--font-mono); font-size:10px; letter-spacing:.06em; text-transform:uppercase; color:var(--text-muted); font-weight:var(--font-weight-medium); }\n        .filter-bar select, .filter-bar input { padding:var(--space-2) var(--space-3); background:var(--bg-secondary); border:1px solid var(--border-light); border-radius:var(--radius-md); color:var(--text-primary); font-size:var(--font-size-sm); }\n        .filter-bar .btn-primary { padding:var(--space-2) var(--space-4); background:var(--accent-primary); color:#fff; border:none; border-radius:var(--radius-md); cursor:pointer; font-weight:var(--font-weight-semibold); align-self:flex-end; }\n        .table-wrapper { overflow-x:auto; background:var(--bg-card); border:1px solid var(--border-light); border-radius:var(--r-lg,var(--radius-lg)); }\n        .log-table { width:100%; border-collapse:collapse; min-width:700px; }\n        .log-table th { text-align:left; padding:var(--space-3); font-family:var(--font-mono); font-size:10px; letter-spacing:.07em; text-transform:uppercase; color:var(--text-muted); font-weight:var(--font-weight-medium); border-bottom:1px solid var(--border-light); position:sticky; top:0; background:var(--bg-secondary); }\n        .log-table td { padding:var(--space-3); border-bottom:1px solid var(--border-light); color:var(--text-secondary); font-size:var(--font-size-sm); vertical-align:top; }\n        .log-table tr:hover td { background:var(--bg-tertiary); cursor:pointer; }\n        .badge { font-family:var(--font-mono); font-size:10px; padding:3px 8px; border-radius:var(--radius-full,999px); border:1px solid; display:inline-flex; align-items:center; gap:5px; font-weight:var(--font-weight-medium); }\n        .badge-auth { color:var(--accent-primary); background:rgba(107,165,201,.13); border-color:rgba(107,165,201,.3); }\n        .badge-create { color:var(--success); background:rgba(95,185,125,.13); border-color:rgba(95,185,125,.3); }\n        .badge-delete { color:var(--danger); background:rgba(229,84,78,.13); border-color:rgba(229,84,78,.3); }\n        .badge-update { color:var(--warning); background:rgba(232,176,75,.13); border-color:rgba(232,176,75,.3); }\n        .badge-default { color:var(--text-muted); background:var(--bg-tertiary); border-color:var(--border-light); }\n        .badge-severity-info { color:var(--info,#6ba5c9); background:rgba(107,165,201,.13); border-color:rgba(107,165,201,.3); }\n        .badge-severity-warning { color:var(--warning); background:rgba(232,176,75,.13); border-color:rgba(232,176,75,.3); }\n        .badge-severity-critical { color:var(--danger); background:rgba(229,84,78,.13); border-color:rgba(229,84,78,.3); animation:badgePulse 1.5s ease-in-out infinite; }\n        @keyframes badgePulse { 0%,100% { box-shadow:0 0 0 0 rgba(229,84,78,.6); } 50% { box-shadow:0 0 0 4px rgba(229,84,78,0); } }\n        .ack-btn { padding:2px 8px; border:1px solid var(--accent-primary); background:transparent; color:var(--accent-primary); border-radius:var(--radius-full,999px); cursor:pointer; font-family:var(--font-mono); font-size:var(--font-size-xs); font-weight:var(--font-weight-medium); }\n        .ack-btn:hover { background:var(--accent-primary); color:#fff; }\n        .alert-row-acked { opacity:0.55; }\n        .alert-row-acked td { color:var(--text-muted); }\n        .ack-info { font-family:var(--font-mono); font-size:var(--font-size-xs); color:var(--text-muted); white-space:nowrap; }\n        .stats-row { display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:var(--space-3); margin-bottom:var(--space-4); }\n        .stat-card { background:var(--bg-card); border:1px solid var(--border-light); border-radius:var(--r-lg,var(--radius-lg)); padding:var(--space-4); display:flex; flex-direction:column; gap:var(--space-2); }\n        .stat-card-label { font-family:var(--font-mono); font-size:10px; letter-spacing:.06em; text-transform:uppercase; color:var(--text-muted); font-weight:var(--font-weight-medium); }\n        .stat-card-value { font-family:var(--font-serif); font-size:var(--font-size-4xl); font-weight:var(--font-weight-semibold); letter-spacing:-.02em; color:var(--text-primary); line-height:1; }\n        .stat-card-value.danger { color:var(--danger); }\n        .stat-card-value.warning { color:var(--warning); }\n        .stat-card-value.muted { color:var(--text-muted); }\n        .stat-card-sub { font-family:var(--font-mono); font-size:var(--font-size-xs); color:var(--text-muted); }\n        .stat-bars { display:flex; align-items:flex-end; gap:2px; height:32px; margin-top:4px; }\n        .stat-bars .bar { flex:1; background:var(--viz-1,var(--accent-primary)); border-radius:2px 2px 0 0; min-height:2px; transition:opacity 0.2s; }\n        .stat-bars .bar:hover { opacity:0.7; }\n        .stat-bars .bar.critical { background:var(--danger); }\n        .stat-distribution { display:flex; gap:4px; height:8px; border-radius:4px; overflow:hidden; background:var(--bg-tertiary); }\n        .stat-distribution > div { transition:flex 0.3s; }\n        .detail-cell { max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:var(--text-muted); }\n        .log-count { font-family:var(--font-mono); color:var(--text-muted); font-size:var(--font-size-xs); letter-spacing:.03em; margin-bottom:var(--space-3); }\n        .empty-state { text-align:center; padding:var(--space-8); color:var(--text-muted); }\n        .empty-state h2 { color:var(--text-secondary); margin-bottom:var(--space-3); }\n\n        .modal-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.6); z-index:1000; justify-content:center; align-items:center; }\n        .modal-overlay.open { display:flex; }\n        .modal { display:block; position:static; height:auto; background:var(--bg-card); border:1px solid var(--border-light); border-radius:var(--r-lg,var(--radius-lg)); width:90%; max-width:650px; max-height:90vh; overflow-y:auto; padding:var(--space-6); }\n        .modal h2 { margin:0 0 var(--space-4); color:var(--text-primary); }\n        .detail-block { background:var(--bg-secondary); padding:var(--space-4); border-radius:var(--radius-md); overflow-x:auto; }\n        .detail-block pre { margin:0; color:var(--text-primary); font-size:var(--font-size-sm); white-space:pre-wrap; word-break:break-all; font-family:var(--font-mono); }\n        .detail-row { margin-bottom:var(--space-3); }\n        .detail-label { font-family:var(--font-mono); font-size:10px; letter-spacing:.06em; text-transform:uppercase; color:var(--text-muted); font-weight:var(--font-weight-medium); margin-bottom:var(--space-1); }\n        .detail-value { color:var(--text-primary); }\n        .modal-actions { display:flex; gap:var(--space-3); justify-content:flex-end; margin-top:var(--space-5); }\n        .modal-actions button { padding:var(--space-2) var(--space-4); border:none; border-radius:var(--radius-md); cursor:pointer; font-weight:var(--font-weight-semibold); }\n        .btn-secondary { background:var(--bg-tertiary); color:var(--text-primary); border:1px solid var(--border-light) !important; }\n        .toast { position:fixed; bottom:20px; right:20px; padding:var(--space-3) var(--space-5); border-radius:var(--radius-md); color:#fff; z-index:2000; opacity:0; transition:opacity .3s; }\n        .toast.show { opacity:1; } .toast.success { background:var(--success); } .toast.error { background:var(--danger); }\n        .loading { text-align:center; padding:var(--space-6); color:var(--text-muted); }" +
                '<\/style>' +
                "<header class=\"page-header\">\n                <button class=\"mobile-menu-btn\" onclick=\"toggleMobileSidebar(event)\">&#9776;</button>\n                <h1><iconify-icon icon=lucide:clipboard-list></iconify-icon> 감사 로그</h1>\n            </header>\n            <div class=\"content-area\" id=\"app\">\n                <div class=\"loading\">권한 확인 중...</div>\n            </div>\n<div class=\"modal-overlay\" id=\"detailModal\">\n        <div class=\"modal\">\n            <h2>로그 상세</h2>\n            <div id=\"detailContent\"></div>\n            <div class=\"modal-actions\"><button class=\"btn-secondary\" onclick=\"closeDetail()\">닫기</button></div>\n        </div>\n    </div>\n<div id=\"toast\" class=\"toast\"></div>" +
                '<\/div>';
        },
        ...
        init: function () {
            try {
                ...
                <!-- Sub-tab nav: audit_logs / alert_history / pool -->
                <div class="filter-tabs">
                    <button class="filter-tab active subtab-btn" data-subtab="logs" onclick="switchAuditSubTab('logs')"><iconify-icon icon=lucide:clipboard-list></iconify-icon> 감사 로그</button>
                    <button class="filter-tab subtab-btn" data-subtab="alerts" onclick="switchAuditSubTab('alerts')"><iconify-icon icon=lucide:bell></iconify-icon> 알림 이력</button>
                    <button class="filter-tab subtab-btn" data-subtab="pool" onclick="switchAuditSubTab('pool')"><iconify-icon icon=lucide:cpu></iconify-icon> LLM Pool</button>
                </div>
                    ...
                    try {
                        ...
                        row.innerHTML = `
                            ...
                            <div class="stat-card">
                                ...
                                <div class="stat-card-value muted" style="font-size:var(--font-size-lg)">총 ${totals7d}건</div>
                                <div class="stat-bars">${trendBars}</div>
                            </div>
                            <div class="stat-card">
