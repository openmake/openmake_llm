...
var ICONS = {
    ...
    fileSearch: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><circle cx="11.5" cy="14.5" r="2.5"/><line x1="13.25" y1="16.25" x2="15" y2="18"/></svg>'
};
    ...
    if (menuLogout) {
        menuLogout.addEventListener('click', function () {
            ...
            if (typeof window.logout === 'function') {
                ...
                ['authToken', 'user', 'guestMode', 'isGuest', 'conversationMemory', 'currentSessionId'].forEach(k => {
                    if (window.SafeStorage) window.SafeStorage.removeItem(k);
...
UnifiedSidebar.prototype._renderConversations = function (conversations) {
    ...
    groupOrder.forEach(function (groupKey) {
        ...
        items.forEach(function (conv) {
            ...
            html += '<div class="us-conv-item' + (isActive ? ' active' : '') + '" data-id="' + esc(id) + '" role="listitem" tabindex="0" aria-label="' + esc(truncate(title, 30)) + '"' + (isActive ? ' aria-current="true"' : '') + '>';
            html += '<span class="us-conv-icon">' + ICONS.chat + '</span>';
            html += '<span class="us-conv-title">' + esc(truncate(title, 30)) + '</span>';
            html += '</div>';
        });
