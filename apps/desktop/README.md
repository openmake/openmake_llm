...
> ⚠️ **동결(freeze) 공지 (2026-08-22)**: 이 Electron 셸은 **신규 기능 추가를 중단**하고
> 보안 수정만 유지한다. 후속은 SwiftUI 네이티브 컴패니언(`apps/desktop-native`, PR #575)이
> 담당하며, 병행 배포 기간을 거쳐 단계적으로 폐기한다(매니페스트 다운로드 집계로 잔존 사용
> 확인 후). UI 는 원격 웹 로드라 동결 후에도 화면은 계속 최신으로 유지된다.
