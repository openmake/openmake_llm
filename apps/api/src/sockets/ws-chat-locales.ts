...
export const WS_PROVIDER_ERROR_MESSAGES: Record<string, Record<ProviderErrorCode, string>> = {
    ko: {
        ...
        SUBSCRIPTION_REQUIRED: '이 모델은 해당 제공자의 유료 구독(상위 플랜)이 필요합니다. 다른 모델을 선택하거나 제공자 사이트에서 플랜을 업그레이드하세요.',
        MODEL_ACCESS_RESTRICTED: '이 모델은 제공자 정책상 이 호출 경로(일반 채팅 API)에서 사용할 수 없습니다. 다른 모델을 선택하세요.',
    ...
    en: {
        ...
        SUBSCRIPTION_REQUIRED: 'This model requires a paid subscription (higher plan) at the provider. Please select a different model or upgrade your plan on the provider\'s site.',
        MODEL_ACCESS_RESTRICTED: 'This model is not available through this access path (general chat API) due to the provider\'s policy. Please select a different model.',
    ...
    ja: {
        ...
        SUBSCRIPTION_REQUIRED: 'このモデルはプロバイダーの有料サブスクリプション（上位プラン）が必要です。別のモデルを選択するか、プロバイダーのサイトでプランをアップグレードしてください。',
        MODEL_ACCESS_RESTRICTED: 'このモデルはプロバイダーのポリシーにより、この呼び出し経路（一般チャットAPI）では利用できません。別のモデルを選択してください。',
    ...
    zh: {
        ...
        SUBSCRIPTION_REQUIRED: '此模型需要该提供商的付费订阅（更高套餐）。请选择其他模型，或在提供商网站升级套餐。',
        MODEL_ACCESS_RESTRICTED: '根据提供商政策，此模型无法通过当前调用路径（通用聊天 API）使用。请选择其他模型。',
    ...
    es: {
        ...
        SUBSCRIPTION_REQUIRED: 'Este modelo requiere una suscripción de pago (plan superior) en el proveedor. Por favor, seleccione un modelo diferente o actualice su plan en el sitio del proveedor.',
        MODEL_ACCESS_RESTRICTED: 'Este modelo no está disponible a través de esta vía de acceso (API de chat general) por la política del proveedor. Por favor, seleccione un modelo diferente.',
    ...
    de: {
        ...
        SUBSCRIPTION_REQUIRED: 'Dieses Modell erfordert ein kostenpflichtiges Abonnement (höherer Tarif) beim Anbieter. Bitte wählen Sie ein anderes Modell oder upgraden Sie Ihren Tarif auf der Anbieterseite.',
        MODEL_ACCESS_RESTRICTED: 'Dieses Modell ist gemäß der Richtlinie des Anbieters über diesen Zugriffsweg (allgemeine Chat-API) nicht verfügbar. Bitte wählen Sie ein anderes Modell.',
    ...
    fr: {
        ...
        SUBSCRIPTION_REQUIRED: 'Ce modèle nécessite un abonnement payant (offre supérieure) chez le fournisseur. Veuillez sélectionner un autre modèle ou mettre à niveau votre offre sur le site du fournisseur.',
        MODEL_ACCESS_RESTRICTED: 'Ce modèle n\'est pas disponible via ce chemin d\'accès (API de chat générale) en raison de la politique du fournisseur. Veuillez sélectionner un autre modèle.',
