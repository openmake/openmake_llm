...
import { GENERAL_SYSTEM_SKILL_NAME, industrySkillName, systemSkillNamesEn } from '../system-skill-names';

const HANGUL = /\p{Script=Hangul}/u;

describe('시스템 스킬·에이전트 영어 표시 이름', () => {
    it('모든 에이전트와 유틸리티 스킬에 한글 없는 영어 이름이 있다', () => {
        const missing = [
            ...Object.values(AGENTS).filter((a) => !a.nameEn || HANGUL.test(a.nameEn)).map((a) => a.id),
            ...[...UTILITY_SKILLS_A, ...UTILITY_SKILLS_B].filter((s) => !s.nameEn || HANGUL.test(s.nameEn)).map((s) => s.id),
        ];
        expect(missing).toEqual([]);
    });

    it('산업·범용·유틸리티 시스템 스킬만 영어로 옮기고 사용자·확장 스킬은 넣지 않는다', () => {
        const names = [industrySkillName('소프트웨어 엔지니어'), GENERAL_SYSTEM_SKILL_NAME, '번역 전문가', 'karpathy-guidelines', '내가 만든 스킬'];
