 ...
 *
 * 아티팩트/지도 의도 턴은 agent_task_list 등 distractor(구 generate_image 는 2026-09-12 도구 자체가 제거돼 픽스처를 교체) 도구를 억제하는데
...
describe('buildExternalToolPlan — 스킬 required 도구 억제 면제', () => {
    const agentTaskList = makeTool('agent_task_list');
    ...
    it('아티팩트 의도면 agent_task_list 를 억제한다 (기존 동작 보존)', () => {
        const plan = buildExternalToolPlan({ ...base, req: artifactReq });
        expect(toolNames(plan)).not.toContain('agent_task_list');
    });

    it('스킬 required 바인딩이면 아티팩트 의도에도 agent_task_list 를 유지한다', () => {
