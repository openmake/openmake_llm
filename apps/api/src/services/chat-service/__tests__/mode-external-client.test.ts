...
describe('resolveModeExternalClient — 해석 우선순위', () => {
    it('guest/userId 없음 → undefined (로컬)', async () => {
        expect(await resolveModeExternalClient(makeResolved('bai', 'q'), undefined, { label: 'Discussion' })).toBeUndefined();
        expect(await resolveModeExternalClient(makeResolved('bai', 'q'), 'guest', { label: 'Discussion' })).toBeUndefined();
    ...
    it('① DeepResearch + research role 이 외부 → role client 우선 (컴포저 선택 무시)', async () => {
        ...
        const out = await resolveModeExternalClient(makeResolved('bai', 'q'), 'u1', { label: 'DeepResearch', modelRole: 'research' });
    ...
    it('② Discussion + 컴포저 명시 외부 선택 → resolveAssignedModelClient 로 외부 client 주입', async () => {
        const out = await resolveModeExternalClient(makeResolved('bai', 'qwen3.8-flash'), 'u1', { label: 'Discussion' });
    ...
    it('② DeepResearch + research role 로컬/미배정 → 컴포저 외부 선택으로 폴백', async () => {
        ...
        const out = await resolveModeExternalClient(makeResolved('bai', 'qwen3.8-flash'), 'u1', { label: 'DeepResearch', modelRole: 'research' });
        expect(out).toBe(externalClientSentinel);
    ...
    it('로컬 선택(local-llm:*) → 단언 없이 그대로 해석 (회귀 0, throw 없음)', async () => {
        ...
        const out = await resolveModeExternalClient(makeResolved('local-llm', 'qwen3.8-27b'), 'u1', { label: 'Discussion' });
...
describe('resolveModeExternalClient — 오류 계약 (명시 외부 선택)', () => {
    it('BYOK 키 미등록/비활성 → ProviderError(MISSING_API_KEY), 로컬 폴백 금지', async () => {
        keyRowResult = null;
        await expect(resolveModeExternalClient(makeResolved('bai', 'qwen3.8-flash'), 'u1', { label: 'Discussion' }))
    ...
    it('카탈로그에 없는 provider → ProviderError(MODEL_NOT_FOUND)', async () => {
        await expect(resolveModeExternalClient(makeResolved('bogus', 'm'), 'u1', { label: 'Discussion' }))
            .rejects.toMatchObject({ code: 'MODEL_NOT_FOUND' });
    });

    it('openai-compatible 아닌 provider → ProviderError(NOT_SUPPORTED)', async () => {
        await expect(resolveModeExternalClient(makeResolved('legacy-anthropic', 'm'), 'u1', { label: 'Discussion' }))
