...
describe('executeExternalTool — 결과 문자열 변환', () => {
    it('text 결과는 이스케이프 없이 원문 그대로 돌려준다', async () => {
        ...
        executeTool.mockResolvedValue({ content: [{ type: 'text', text }] });
    ...
    it('여러 text 는 줄바꿈으로 잇고 비텍스트 항목은 JSON 으로 남긴다', async () => {
        ...
        executeTool.mockResolvedValue({ content: [{ type: 'text', text: 'first' }, image, { type: 'text', text: 'second' }] });
    ...
    it('상한을 넘으면 MAX_TOOL_RESULT_CHARS 에서 자른다', async () => {
        executeTool.mockResolvedValue({ content: [{ type: 'text', text: 'x'.repeat(MAX_TOOL_RESULT_CHARS + 500) }] });
    ...
    it('상한 밖에 있는 kakaomap 블록은 앞에 따로 붙여 보존한다', async () => {
        ...
        executeTool.mockResolvedValue({ content: [{ type: 'text', text: `${'y'.repeat(MAX_TOOL_RESULT_CHARS)}\n${block}` }] });
