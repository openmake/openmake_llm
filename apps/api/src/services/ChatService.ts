...
export class ChatService {
    ...
    private async getAllowedTools(reqCtx: RequestContext): Promise<ToolDefinition[]> {
        ...
        const mcpSelectHints = getChatTurnIntegrations().map((i) => i.mcpSelectionHint?.(reqCtx)).filter(Boolean);
        const mcpSelectMessage = mcpSelectHints.length > 0 ? `${reqCtx.message ?? ''} ${mcpSelectHints.join(' ')}` : reqCtx.message;
        ...
        // 상시 노출·의도 턴 강제 포함은 순수 함수로(F26.2 — 도구 선택 평가가 같은 판정을 쓴다)
        const finalCombined = selectTurnTools({
            allTools, merged, userMcpAutoOn, message: reqCtx.message, tailWebGround: reqCtx.tailWebGround,
        });
