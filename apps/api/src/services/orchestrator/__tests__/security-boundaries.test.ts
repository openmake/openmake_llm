...
describe('코드 기본값 끄기 스위치 — CAPABILITY_DEFAULT_IMAGE_GENERATE', () => {
    ...
    it('미설정·빈 값·none 이면 기본값 없음(→ 미배정 명시 실패), 값이 있으면 그 값', () => {
        delete process.env.CAPABILITY_DEFAULT_IMAGE_GENERATE; expect(load()['image.generate']).toBeUndefined(); // 코드 기본값 없음(2026-09-18 제거)
