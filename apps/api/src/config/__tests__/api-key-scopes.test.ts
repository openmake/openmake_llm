...
describe('apiKeyHasScope', () => {
    ...
    test('미보유 스코프는 거부', () => {
        ...
        // add-on 이 기여한 스코프도 같은 규칙 — 다른 축의 키로는 통과하지 못한다.
        expect(apiKeyHasScope(['chat'], 'some-addon-scope')).toBe(false);
        expect(apiKeyHasScope(['some-addon-scope'], API_KEY_SCOPES.CHAT)).toBe(false);
        expect(apiKeyHasScope(['some-addon-scope'], 'some-addon-scope')).toBe(true);
    });
