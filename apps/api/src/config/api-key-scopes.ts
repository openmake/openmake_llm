 ...
 *
 * 켜진 add-on 이 자기 스코프를 더 얹는다(addon-host/contributions) — 예: 봇 프로세스가 자기 운영 설정을 받아오는 축.
 *
 * @module config/api-key-scopes
 */

import { contributedApiKeyScopes } from '../addon-host/contributions';
...
/** Base 스코프 또는 add-on 이 기여한 스코프 문자열 */
export type ApiKeyScope = typeof API_KEY_SCOPES[keyof typeof API_KEY_SCOPES] | (string & {});

/** 발급 시 허용하는 스코프 문자열 화이트리스트 (미지 스코프 거부 — 오타·권한 오해 방지). */
export const ALLOWED_API_KEY_SCOPES: ReadonlySet<string> = new Set([
    '*', API_KEY_SCOPES.BRIDGE, API_KEY_SCOPES.CHAT, ...contributedApiKeyScopes(),
]);
