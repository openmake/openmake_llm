...
export const MEMORY_EXTRACTION = {
     ...
     * 실측: 실제 중복 0.80~0.83 / 대립 쌍(파이썬↔자바·김철수↔김영희·코스모스↔비트코인·TypeScript↔Python) 0.33~0.67.
     */
    ...
    /**
     * 어절 끝 조사·어미(크루드 스테밍 — 형태소 분석 없이 결정적으로 반복 제거, 최대 dupTokenStripRounds 회).
     * 라이브 실측(2026-09-06): "받기를"→"받기", "받는"→"받" 으로 1회 제거는 어간이 안 맞았다 → 반복 + "기" 추가.
     */
    dupTokenSuffix: /(으로|에서|에게|까지|부터|처럼|하기를|하기|하는|하고|했다|한다|합니다|입니다|이다|있다|은|는|이|가|을|를|의|로|와|과|도|에|고|다|해|기)$/u,
    dupTokenStripRounds: 3,
    /** 근접 중복 비교에서 무시할 짧은 토큰 길이 — 반복 제거 후 1자 어간("받")도 신호라 1. */
    dupTokenMinLen: 1,
    /** 근접 중복 비교에서 제외하는 수식어(정도·빈도 부사) — 같은 사실의 합집합만 키운다("모든 코드 예시" vs "코드 예시"). */
    dupStopwords: new Set(['모든', '항상', '주로', '앞으로', '매우', '아주', '계속', '보통', '특히', '현재', '약간', '좀', '꼭', '가장', '다시', '대체로', '대부분', '많이', '자주']),
    /**
