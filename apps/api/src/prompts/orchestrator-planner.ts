 ...
 */
import { PLANNABLE_CAPABILITIES, CAPABILITY_PLANNER_HINTS, VIDEO_GEN_ASPECT_SIZES, PLAN_CONVERSATION_TEXT_MARKER, type Capability } from '../config/capabilities';
...
- music.generate: instruction 에는 장르·분위기·악기·템포를 영어로 씁니다. 부를 가사가 사용자 메시지나 직전 답변(아티팩트 포함)에 이미 있으면 **가사를 옮겨 적지 말고** input.lyrics 에 "${PLAN_CONVERSATION_TEXT_MARKER}" 만 적습니다(실행기가 대화에서 원문을 가져옵니다 — 가사를 옮겨 적으면 계획이 길어져 잘립니다). 가사를 새로 지어야 하면 먼저 text.reason 작업이 가사만 쓰게 한 뒤 input.refs 로 넘깁니다. 연주곡이면 lyrics 를 비웁니다. 사용자가 길이를 말하면 input.duration 에 초 숫자를 넣습니다(예: "60").
...
- music.generate: the instruction describes genre, mood, instruments and tempo in English. If the lyrics to sing are already in the user message or a previous answer (including artifacts), **do not copy them** — put just "${PLAN_CONVERSATION_TEXT_MARKER}" in input.lyrics (the executor takes the original from the conversation; copying makes the plan long enough to be cut off). If lyrics must be written, first have a text.reason task write only the lyrics and pass it via input.refs. Leave lyrics empty for instrumentals. If the user states a length, put the number of seconds in input.duration (e.g. "60").
