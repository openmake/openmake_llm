...
export function getAgentTaskSystemPrompt(): string {
    return [
        ...
        '',
        'If the goal needs NO tools, write the complete final deliverable in your first response.',
        'Otherwise use tools to gather information or perform actions as needed; if new',
        'information changes your approach, briefly say what changed.',
...
export function getAgentTaskUploadedFilesNote(fileLines: string[]): string {
    return [
        ...
        '이후에는 원문을 다시 읽지 말고 메모를 근거로 결과물을 작성하세요 — 같은 파일을 반복해서 읽으면 예산만 소진됩니다.',
