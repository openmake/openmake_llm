...
- \`csv\` — CSV 데이터 (헤더 행 포함, 표로 자동 렌더, 사용자가 xlsx 로 내려받을 수 있음)
...
**🎨 OpenMake 디자인 시스템 (html 기본값)**: 사용자가 다른 스타일을 지정하지 않으면 문서 맨 끝 "html 디자인 최종 점검"의 \`:root\` 토큰 블록을 그대로 사용하세요. 우선순위: 사용자 지정 > 이 시스템 > 임의 선택.
...
**🎨 OpenMake design system (html default)**: unless the user specifies another style, use the \`:root\` token block from the "html design final check" section at the end of this document verbatim. Precedence: user request > this system > arbitrary choice.
