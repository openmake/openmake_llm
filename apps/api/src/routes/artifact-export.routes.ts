/**
 * Artifact Export Routes — 아티팩트의 pdf/docx/xlsx 변환 (P1 보고서 파이프라인 Phase 3, xlsx 는 F20.1).
 *
 ...
 * - xlsx: csv 아티팩트 본문 또는 reportdata source_data 에서 openpyxl 생성 (그 외 409)
...
router.post('/sessions/:sid/artifacts/:aid/export', requireAuth, artifactExportLimiter, asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!isExportFormat(format)) {
        res.status(400).json({ error: 'INVALID_FORMAT', detail: `format 은 ${EXPORT_FORMATS.join(' | ')}` });
        return;
    }
    ...
    try {
        const sourceData = format === 'pdf' || (format === 'xlsx' && latest.kind === 'csv') ? null : await repo.getLatestSourceData(sid, aid);
        const result = await exportByFormat(format, latest, sourceData);

        void getAuditService().logAudit({
            action: 'artifact_export',
            userId,
            details: {
                sessionId: sid, artifactId: aid, format,
...
router.post('/agent-tasks/:taskId/artifacts/:aid/export', requireAuth, artifactExportLimiter, asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        const result = await exportByFormat(format, found, found.sourceData);

        void getAuditService().logAudit({
            action: 'artifact_export',
            userId,
            details: { taskId, artifactId: aid, format, durationMs: result.durationMs },
            actor: { email: req.user?.email, role: req.user?.role },
        });
