...
import { resolveCapabilityTarget } from '../services/orchestrator/capability-resolver';
...
router.get('/models', optionalAuth, asyncHandler(async (req: Request, res: Response) => {
    ...
    // 이미지 생성 모델 — 채팅 모델이 아니라 오케스트레이터 image.generate capability 배정으로 호출.
    // UI 는 이 값으로 "이미지 생성 가능" 표시만 한다 (채팅 셀렉터 옵션 아님). 미배정 시 null.
    const imageModel = await resolveCapabilityTarget('image.generate', userId ?? undefined)
        .then((t) => t.fullId)
        .catch(() => null);
