...
/**
 * 주제 분해 프롬프트.
 * 2026-09-05 (gpt-researcher 대조, PR): ① 검색 연산자 금지 — 네이버·다음·SearXNG 는 `site:`/`OR` 등을
 * 지원하지 않아 빈 결과가 난다 ② 현재 날짜 주입 — "최신 동향" 검색어가 연도를 못 잡던 것.
 * `today` 는 YYYY-MM-DD, 미지정 시 호출 시점.
 */
export function getDecomposePrompt(lang: string, topic: string, today: string = new Date().toISOString().slice(0, 10)): string {
    const prompts: Record<string, string> = {
        ko: `다음 주제를 심층 연구하기 위해 8-15개의 서브 토픽을 생성하세요.\n주제: ${topic}\n\n요구사항:\n1) 각 서브 토픽마다 서로 다른 관점의 검색어 2-3개를 만드세요.\n2) 각 검색어는 1-6개 단어로 간결하게 작성하고, 서로 거의 동일한 검색어는 만들지 마세요.\n3) 중요도는 1-5 정수로 부여하세요.\n4) 검색 연산자(site:, filetype:, inurl:, intitle:, OR, AND, NOT)는 쓰지 마세요 — 일부 검색 엔진은 지원하지 않아 빈 결과가 납니다. 자연어 구만 사용하세요.\n5) 현재 날짜는 ${today} 입니다. 최신 동향·통계 검색어에는 이 연도를 반영하세요.\n6) JSON 배열만 출력하세요. 설명 문장 금지.\n\n반드시 다음 형식:\n[\n  {\n    "title": "서브 토픽 제목",\n    "searchQueries": ["검색어 1", "검색어 2", "검색어 3"],\n    "importance": 5\n  }\n]`,
        ja: `次のトピックを深く研究するために8-15個のサブトピックを生成してください。\nトピック: ${topic}\n\n要件:\n1) 各サブトピックに異なる観点の検索クエリ2-3個を含めてください。\n2) 各クエリは1-6語で簡潔に。ほぼ同一のクエリは作らないこと。\n3) 重要度は1-5の整数で。\n4) 検索演算子(site:, filetype:, inurl:, intitle:, OR, AND, NOT)は使わないこと — 一部の検索エンジンは未対応で空の結果になる。自然言語の句のみ。\n5) 現在の日付は ${today}。最新動向・統計のクエリにはこの年を反映すること。\n6) JSON配列のみ出力。説明文は不要。\n\n必ず次の形式で:\n[\n  {\n    "title": "サブトピック名",\n    "searchQueries": ["クエリ1", "クエリ2", "クエリ3"],\n    "importance": 5\n  }\n]`,
        zh: `为以下主题生成8-15个子主题进行深入研究。\n主题: ${topic}\n\n要求:\n1) 每个子主题包含2-3个不同角度的搜索查询。\n2) 每个查询用1-6个词简洁表达，禁止近乎相同的查询。\n3) 重要性为1-5的整数。\n4) 不要使用搜索运算符(site:, filetype:, inurl:, intitle:, OR, AND, NOT)——部分搜索引擎不支持，会返回空结果。只用自然语言短语。\n5) 当前日期为 ${today}。最新动态、统计类查询请体现该年份。\n6) 仅输出JSON数组，禁止说明文字。\n\n必须使用以下格式:\n[\n  {\n    "title": "子主题标题",\n    "searchQueries": ["查询1", "查询2", "查询3"],\n    "importance": 5\n  }\n]`,
        es: `Genera 8-15 subtemas para investigar en profundidad el siguiente tema.\nTema: ${topic}\n\nRequisitos:\n1) Cada subtema debe incluir 2-3 consultas de búsqueda diversas.\n2) Cada consulta debe ser concisa (1-6 palabras); evita consultas casi idénticas.\n3) La importancia debe ser un entero de 1-5.\n4) No uses operadores de búsqueda (site:, filetype:, inurl:, intitle:, OR, AND, NOT): algunos motores no los admiten y devuelven resultados vacíos. Solo frases en lenguaje natural.\n5) La fecha actual es ${today}. Refleja este año en las consultas sobre tendencias recientes y estadísticas.\n6) Solo genera un array JSON, sin texto adicional.\n\nFormato requerido:\n[\n  {\n    "title": "Título del subtema",\n    "searchQueries": ["consulta 1", "consulta 2", "consulta 3"],\n    "importance": 5\n  }\n]`,
        de: `Erstelle 8-15 Unterthemen für eine Tiefenrecherche zum folgenden Thema.\nThema: ${topic}\n\nAnforderungen:\n1) Jedes Unterthema muss 2-3 verschiedene Suchanfragen enthalten.\n2) Jede Anfrage muss knapp sein (1-6 Wörter); nahezu identische Anfragen vermeiden.\n3) Wichtigkeit als Ganzzahl von 1-5.\n4) Keine Suchoperatoren (site:, filetype:, inurl:, intitle:, OR, AND, NOT) verwenden — manche Suchmaschinen unterstützen sie nicht und liefern leere Ergebnisse. Nur natürlichsprachliche Phrasen.\n5) Das aktuelle Datum ist ${today}. Bei Anfragen zu aktuellen Trends und Statistiken dieses Jahr berücksichtigen.\n6) Nur JSON-Array ausgeben, kein zusätzlicher Text.\n\nErforderliches Format:\n[\n  {\n    "title": "Unterthema-Titel",\n    "searchQueries": ["Anfrage 1", "Anfrage 2", "Anfrage 3"],\n    "importance": 5\n  }\n]`,
        fr: `Générez 8 à 15 sous-thèmes pour une recherche approfondie sur le sujet suivant.\nSujet : ${topic}\n\nExigences :\n1) Chaque sous-thème doit inclure 2-3 requêtes de recherche variées.\n2) Chaque requête doit être concise (1 à 6 mots) ; évitez les requêtes quasi identiques.\n3) L'importance doit être un entier de 1 à 5.\n4) N'utilisez pas d'opérateurs de recherche (site:, filetype:, inurl:, intitle:, OR, AND, NOT) : certains moteurs ne les prennent pas en charge et renvoient des résultats vides. Uniquement des expressions en langage naturel.\n5) La date actuelle est ${today}. Reflétez cette année dans les requêtes sur les tendances récentes et les statistiques.\n6) Produisez uniquement un tableau JSON, sans texte supplémentaire.\n\nFormat requis :\n[\n  {\n    \"title\": \"Titre du sous-thème\",\n    \"searchQueries\": [\"requête 1\", \"requête 2\", \"requête 3\"],\n    \"importance\": 5\n  }\n]`,
    };
    return prompts[lang] || `Generate 8-15 subtopics for deep research on this topic.\nTopic: ${topic}\n\nRequirements:\n1) Each subtopic must include 2-3 diverse search queries.\n2) Keep each query concise (1-6 words); never produce near-identical queries.\n3) importance must be an integer from 1-5.\n4) Do not use search operator syntax (site:, filetype:, inurl:, intitle:, OR, AND, NOT) — some search backends do not support them and return empty results. Use plain natural-language phrases only.\n5) Assume the current date is ${today}. Reflect this year in queries about recent trends and statistics.\n6) Output JSON array only with no additional text.\n\nRequired format:\n[\n  {\n    "title": "Subtopic title",\n    "searchQueries": ["query 1", "query 2", "query 3"],\n    "importance": 5\n  }\n]`;
}
...
const RESEARCH_MESSAGES: Record<string, Record<string, string>> = {
    ...
    loopSynthChunk: {
        ko: '루프 {loop}: 정보 합성 중 ({completed}/{total} 청크)',
        en: 'Loop {loop}: Synthesizing ({completed}/{total} chunks)',
        ja: 'ループ {loop}: 情報を合成中 ({completed}/{total}チャンク)',
        zh: '循环 {loop}: 正在合成信息 ({completed}/{total}块)',
        es: 'Bucle {loop}: Sintetizando ({completed}/{total} fragmentos)',
        de: 'Schleife {loop}: Synthese ({completed}/{total} Blöcke)',
        fr: 'Boucle {loop} : Synthèse ({completed}/{total} blocs)',
    },
    loopSynthMerging: {
        ko: '루프 {loop}: 합성 결과 병합 중...',
        en: 'Loop {loop}: Merging synthesis...',
        ja: 'ループ {loop}: 合成結果を統合中...',
        zh: '循环 {loop}: 正在合并合成结果...',
        es: 'Bucle {loop}: Combinando síntesis...',
        de: 'Schleife {loop}: Synthese wird zusammengeführt...',
        fr: 'Boucle {loop} : Fusion de la synthèse...',
    },
    generatingReport: {
        ko: '최종 보고서 생성 중...',
