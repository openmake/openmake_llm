...
- `omk dev setup|up` 을 인스턴스 이름 `local` 로 바꾼 뒤 **실기동은 아직 안 했다**(순수 테스트만) — 검증 호스트의 작업 클론에 `openmake_bench` 가 없고 환경 dev 가 떠 있어 미뤘다
- 백업은 레포의 `db-backup.sh` 로 통일했다(`omk env backup`). 운영 호스트가 쓰던 레포 밖 스크립트가 더 하던 일(원격 복사·알림 등)이 있는지는 **확인하지 못했다** — 있으면 레포 쪽에 합친다
