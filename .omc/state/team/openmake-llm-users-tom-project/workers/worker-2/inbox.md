...
1. Claim your task:
   omc team api claim-task --input '{"team_name":"openmake-llm-users-tom-project","task_id":"2","worker":"worker-2"}' --json
   Save the claim_token from the response.
2. Do the work described below.
3. On completion (use claim_token from step 1):
   omc team api transition-task-status --input '{"team_name":"openmake-llm-users-tom-project","task_id":"2","from":"in_progress","to":"completed","claim_token":"<claim_token>"}' --json
4. On failure (use claim_token from step 1):
   omc team api transition-task-status --input '{"team_name":"openmake-llm-users-tom-project","task_id":"2","from":"in_progress","to":"failed","claim_token":"<claim_token>"}' --json
5. Exit immediately after transitioning.

## Task Assignment
Task ID: 2
Worker: worker-2
Subject: Worker 2: OpenMake LLM 프로젝트(/Users/tom/projects/development/openmake_llm) Phase 
