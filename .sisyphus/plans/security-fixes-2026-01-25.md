...
### Task 6: DB ON DELETE CASCADE 추가
- [ ] 마이그레이션 스크립트 작성
...
- [ ] 마이그레이션 실행 스크립트 작성
  - **파일**: `database/migrate.ts` (신규)
  - **작업**: SQLite 마이그레이션 러너 구현
  - **예상 시간**: 1시간
  - **병렬 가능**: ✅ Task 6-1과 병렬

- [ ] 테스트 데이터로 CASCADE 동작 검증
  - **작업**:
    1. 테스트 유저 생성
    2. 관련 세션/리서치 데이터 생성
    3. 유저 삭제
...
### Task 7: 데이터베이스 코드 중복 제거
- [ ] `backend/api/src/data/models/unified-database.ts` 삭제
...
### Task 8: JWT URL → HTTP-only Cookie 변경
- [ ] 백엔드: OAuth 콜백 수정 (토큰을 쿠키로 설정)
...
### Task 9: Rate Limiting 적용
- [ ] `OllamaClient.chat` 메서드 수정: 요청 전 한도 체크
...
- [ ] `OllamaClient.generate` 메서드에도 동일 적용
  - **파일**: `backend/api/src/ollama/client.ts`
  - **예상 시간**: 15분
  - **병렬 가능**: ✅ Task 9-1과 병렬
...
### Task 10: server.ts 리팩토링 (Controller 분리)
- [ ] AdminController 생성
...
### Task 15: TypeScript `any` 타입 제거
- [ ] `server.ts`의 `any` 타입 찾기 및 대체
