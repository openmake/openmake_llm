...
| `mcp/` | Model Context Protocol: tool router, external client, server registry, user sandbox |
...
- **MCP Tools**: 13 built-in tools (`mcp/tools.ts` `builtInTools`) — 모든 이용자에게 제한 없이 제공 (구독/tool tier 게이팅 전면 제거, 2026-06-19). Web scraping tools (web_scrape, web_map, web_crawl) are always active (no API key required).
